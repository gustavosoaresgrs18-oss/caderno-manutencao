...
function abrirModalKm(gpsDist) {
  ...
  atualizarKmVivo();
  // a leitura da foto anterior nao pode sobrar na tela do dia seguinte
  const _oc = document.getElementById('ocrKmCand');
  if (_oc) { _oc.style.display = 'none'; _oc.innerHTML = ''; }
  if (typeof mostrarBotoesFoto === 'function') mostrarBotoesFoto();
...
function editarAbastecimento(id) {
  ...
  // cada abertura comeca limpa: aviso de leitura antiga confunde
  const _oc2 = document.getElementById('ocrCupomAviso');
  if (_oc2) { _oc2.style.display = 'none'; _oc2.innerHTML = ''; }
  if (typeof mostrarBotoesFoto === 'function') mostrarBotoesFoto();
...
// ═══════════════════════════════════════════════════════════════
//  LER PELA FOTO (v4.07) — odômetro e cupom da bomba
//  ─────────────────────────────────────────────────────────────
//  O ML Kit roda NO APARELHO: de graça, offline, sem servidor e sem
//  conta de API. Nada do que ele lê sai do celular.
//
//  ⚠️ A REGRA QUE MANDA AQUI: a foto PREENCHE, o motorista CONFIRMA.
//  Nunca salva sozinho. É a lição que o GPS já ensinou neste app —
//  "motorista exausto confirmava e o dia inteiro virava 15 km".
//  Preencher automático com número que pode estar errado é pior do
//  que não preencher nada.
//
//  ⚠️ E O DIFÍCIL NÃO É LER DÍGITO — é escolher QUAL número. O painel
//  tem velocidade, RPM, relógio, combustível, hodômetro parcial e
//  hodômetro total ao mesmo tempo. A vantagem do Copiloto é que ele
//  JÁ SABE o número esperado: o último odômetro mais um dia de
//  trabalho. Não é leitura às cegas, é leitura com palpite forte.
// ═══════════════════════════════════════════════════════════════
...
// ── todos os números que aparecem na foto, já limpos ────────────
// O ML Kit devolve "105.712", "105 712", "1O5712" (letra O no lugar do zero).
// Aqui vira número, e o que não vira é descartado.
function _numerosDaFoto(res) {
  if (!res) return [];
  const bruto = String(res.text || '');
  const achados = [];
  bruto.split(/\s+/).forEach(function (p) {
    const limpo = p.replace(/[Oo]/g, '0').replace(/[Il|]/g, '1').replace(/[^\d.,]/g, '');
    if (!limpo) return;
    // sem separador nenhum: é um inteiro
    const soDigitos = limpo.replace(/[.,]/g, '');
    if (!soDigitos || soDigitos.length > 9) return;
    achados.push({ texto: p, inteiro: Number(soDigitos), comSeparador: /[.,]/.test(limpo), limpo: limpo });
  });
  return achados;
}

// ── ODÔMETRO ────────────────────────────────────────────────────
// ⚠️ O hodômetro PARCIAL é a armadilha: fica colado no total, tem a
// mesma cara, e é o que a maioria zera toda semana. Pegar o parcial
// destrói a cadeia de km inteira. O que separa os dois é o palpite:
// o total tem que ser >= o último odômetro conhecido.
const OCR_KM_MAX_DIA = 1200;      // teto de um dia de trabalho, folgado
function acharOdometroNaFoto(res, ultimoOdo) {
  const nums = _numerosDaFoto(res)
    .filter(function (n) { return n.inteiro >= 100; })   // relógio e marcha não interessam
    .map(function (n) { return n.inteiro; });
  if (!nums.length) return { valor: null, candidatos: [] };

  if (ultimoOdo != null && ultimoOdo > 0) {
    // ⚠️ A janela cresce com os dias sem registrar. Quem some por uma
    // semana volta com 1.500 km a mais, e uma janela fixa recusaria o
    // número CERTO — mandando ele digitar do mesmo jeito.
    const dias = (typeof diasDesdeUltimoRegistro === 'function') ? Math.max(1, diasDesdeUltimoRegistro()) : 1;
  ...
  // aqui carimba a base de todas as contas seguintes. Devolve os candidatos
  // pro motorista apontar qual é (Regra Sagrada nº 2).
  const plausiveis = nums.filter(function (v) { return v >= 1000 && v <= 9999999; })
    .filter(function (v, i, a) { return a.indexOf(v) === i; })
    .sort(function (a, b) { return b - a; }).slice(0, 4);
  return { valor: null, candidatos: plausiveis };
}

// ── CUPOM DA BOMBA ──────────────────────────────────────────────
// ⚠️ Não procura rótulo ("VALOR", "LITROS") de propósito: cada bomba
// escreve de um jeito, e cupom amassado perde justamente a palavra.
// O que não muda é a ARITMÉTICA: valor = litros × preço. Então o app
// testa as combinações e aceita a que fecha a conta. Se nenhuma fecha,
// ele não inventa.
// O preço por litro que ELE costuma pagar. É o palpite forte do cupom, igual
// ao último odômetro é o palpite forte do painel.
function precoLitroConhecido() {
  try {
    const h = lerLS('historicoAbastecimentos', [])
      .map(function (r) { return Number(r.ppl); })
      .filter(function (v) { return isFinite(v) && v >= 2 && v <= 12; })
      .slice(0, 6);
    if (!h.length) return null;
    const ord = h.slice().sort(function (a, b) { return a - b; });
    return ord[Math.floor(ord.length / 2)];   // mediana: um posto caro não desloca tudo
  } catch (e) { return null; }
}
function acharAbastecimentoNaFoto(res) {
  const nums = _numerosDaFoto(res)
    .filter(function (n) { return n.comSeparador; })          // dinheiro sempre tem vírgula
    .map(function (n) {
      const t = n.limpo.replace(/\./g, ',');
      const partes = t.split(',');
      const dec = partes.length > 1 ? partes[partes.length - 1] : '';
      if (dec.length > 3) return null;                         // "105.712" é km, não dinheiro
      const v = Number(partes.slice(0, -1).join('') + '.' + dec);
      return isFinite(v) ? v : null;
    })
    .filter(function (v) { return v !== null && v > 0 && v < 100000; });

  const unicos = nums.filter(function (v, i, a) { return a.indexOf(v) === i; });
  const pref = precoLitroConhecido();

  // ⚠️ AMBIGUIDADE REAL: em "45,00 = 6,199 × 7,26" os DOIS últimos cabem como
  // preço de litro, e as duas leituras fecham a conta. Sem desempate o app
  // escolheria no chute e poderia gravar 7,26 como preço da gasolina.
  // O desempate é o histórico dele: entre duas contas válidas, vence a que tem
  // o preço mais perto do que ele costuma pagar.
  let melhor = null;
  for (let i = 0; i < unicos.length; i++) {
    for (let j = 0; j < unicos.length; j++) {
      for (let k = 0; k < unicos.length; k++) {
        if (i === j || j === k || i === k) continue;
        const valor = unicos[i], litros = unicos[j], preco = unicos[k];
        if (preco < 2 || preco > 12) continue;
        if (litros < 0.5 || litros > 200) continue;
        if (valor < 3 || valor > 3000) continue;
        const erro = Math.abs(litros * preco - valor) / valor;
        if (erro > 0.02) continue;                             // 2% cobre o arredondamento da bomba
        const distPreco = (pref !== null) ? Math.abs(preco - pref) : 0;
        const nota = distPreco * 10 + erro;                    // preço parecido pesa mais que decimal exato
        if (!melhor || nota < melhor.nota) melhor = { valor: valor, litros: litros, preco: preco, nota: nota };
      }
    }
  }
  ...
  for (let i = 0; i < unicos.length; i++) {
    for (let j = 0; j < unicos.length; j++) {
      if (i === j) continue;
      const valor = unicos[i], preco = unicos[j];
      if (valor < 3 || valor > 3000) continue;
      if (Math.abs(preco - pref) / pref > 0.15) continue;      // 15% já cobre variação de posto
      const litros = valor / preco;
      if (litros >= 0.5 && litros <= 200) {
        return { valor: valor, litros: Math.round(litros * 100) / 100, preco: preco, completo: false };
      }
    }
  }
  return null;
...
function mostrarBotoesFoto() {
  const tem = ocrDisponivel();
  ['btnFotoOdo', 'btnFotoCupom'].forEach(function (id) {
    const b = document.getElementById(id);
    if (b) b.style.display = tem ? 'flex' : 'none';
  });
}
...
function _ocrTrabalhando(id, ligado, textoNormal) {
  ...
  if (!b) return;
  b.disabled = !!ligado;
  b.classList.toggle('lendo', !!ligado);
  b.innerHTML = ligado
    ? 'Lendo a foto...'
    : '<svg class="ico"><use href="#i-camera"></use></svg> ' + textoNormal;
}

// ── botão do odômetro (modal de fechar o dia) ───────────────────
async function lerOdometroPelaFoto() {
  const cx = document.getElementById('ocrKmCand');
  if (cx) { cx.style.display = 'none'; cx.innerHTML = ''; }
  _ocrTrabalhando('btnFotoOdo', true);
  let res = null;
  try { res = await fotografarELer(); } catch (e) { res = null; }
  _ocrTrabalhando('btnFotoOdo', false, 'Ler pela foto do painel');
  if (!res) return;

  const r = acharOdometroNaFoto(res, ultimoOdoAtivo());

  if (r.valor != null) {
    const inp = document.getElementById('inputKm');
    if (inp) { inp.value = fmtKm(r.valor); if (typeof atualizarKmVivo === 'function') atualizarKmVivo(); }
    // ⚠️ NÃO confirma sozinho. O aviso existe pra ele BATER O OLHO no painel
    // antes de tocar em Confirmar — foto de painel sujo troca 8 por 3, e um
    // dígito errado aqui contamina o custo por km de todos os dias seguintes.
    if (cx) {
      cx.style.display = 'block';
      cx.innerHTML = '<div class="ocr-ok">Li <b class="num">' + fmtKm(r.valor)
        + '</b> km no painel. Confira antes de confirmar.</div>';
    }
    return;
  }

  // não deu pra escolher sozinho: mostra o que leu e deixa ele apontar
  if (r.candidatos && r.candidatos.length) {
    if (cx) {
      cx.style.display = 'block';
      cx.innerHTML = '<div class="ocr-pergunta">Achei mais de um número. Qual é o odômetro?</div>'
        + '<div class="ocr-opcoes">'
        + r.candidatos.map(function (v) {
            return '<button type="button" class="ocr-op num" data-v="' + v + '">' + fmtKm(v) + '</button>';
          }).join('')
        + '</div>';
...
async function lerCupomPelaFoto() {
  const cx = document.getElementById('ocrCupomAviso');
  if (cx) { cx.style.display = 'none'; cx.innerHTML = ''; }
  _ocrTrabalhando('btnFotoCupom', true);
  let res = null;
  try { res = await fotografarELer(); } catch (e) { res = null; }
  _ocrTrabalhando('btnFotoCupom', false, 'Ler o cupom pela foto');
  if (!res) return;
  ...
  if (iv) iv.value = numBRL(r.valor);
  if (il) il.value = String(r.litros).replace('.', ',');
  if (typeof calcCustoPorKmTela === 'function') calcCustoPorKmTela();

  // ⚠️ Diz de onde veio cada número. Quando o app deduziu os litros (não
  // estavam escritos no cupom), ele avisa — senão o motorista confia num
  // litro que ninguém leu, e a média de consumo nasce torta.
  if (cx) {
    cx.style.display = 'block';
    cx.innerHTML = '<div class="ocr-ok">Li <b class="num">' + fmtBRL(r.valor) + '</b> · <b class="num">'
      + String(r.litros).replace('.', ',') + ' L</b> a <b class="num">' + fmtBRL(r.preco) + '</b>/L.'
...
// ⚠️ A PONTE PARA QUALQUER PLUGIN NATIVO. Este app não tem bundler: os
// arquivos entram por <script src> e o copiar-para-www.js copia só o app. O
// pacote JS de um plugin mora em node_modules e NUNCA chega no www/ — então
// `Capacitor.Plugins.X`, que é criado por esse pacote, não existe.
//
// Quem registra o plugin de verdade é a ponte NATIVA, e `Capacitor.PluginHeaders`
// é a lista do que o Android realmente expõe. Com o nome de lá, `registerPlugin`
// devolve o proxy que fala com o código nativo, sem import nenhum.
//
// ⚠️ Era uma função só pro LocalNotifications. Virou geral quando entrou o
// segundo plugin — antes que virasse a terceira cópia da mesma regra, que é o
// erro que já custou caro sete vezes neste projeto.
const _plugCache = {};
function plugNativo(nome) {
  if (_plugCache[nome]) return _plugCache[nome];
  try {
    const C = window.Capacitor;
    if (!C) return null;
    if (C.Plugins && C.Plugins[nome]) { _plugCache[nome] = C.Plugins[nome]; return _plugCache[nome]; }
    const temNativo = Array.isArray(C.PluginHeaders)
      && C.PluginHeaders.some(function (h) { return h && h.name === nome; });
    if (temNativo && typeof C.registerPlugin === 'function') {
      _plugCache[nome] = C.registerPlugin(nome);
      return _plugCache[nome];
    }
  } catch (e) {}
  return null;
}
function _notifPlugin() { return plugNativo('LocalNotifications'); }
